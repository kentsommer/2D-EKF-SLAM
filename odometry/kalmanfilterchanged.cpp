...
void KalmanFilter::doPropagation(double dt, std::ofstream& file,std::ofstream& knownfeaturesFile) {
  // std::cout << "\nX_hat_min =" << std::endl;
...
//   Set = Propagate(x_hat_min, m, 0.1, 0.01, Q, 1);
  ...
  int knownfeaturenumber= sqrt(Set.size())-3;
  if knownfeaturenumber>0
  {
  	for (int a=1;a<knownfeaturenumber;a=a+1)
  		{
  			knownfeaturesFile << Set(3+a,0) << " "<< Set(4+a,0)<<std::endl;
  		}
